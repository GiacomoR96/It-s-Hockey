 ...
 */
    ...
    textDEBUG = this.add.text(0, 50, '', { font: '33px Courier', fill: '#000000' });
/*
    if(app.idPorta==0){
...
//    puck.setAcceleration(100,100);
//    puck.body.applyForce([100,100]);

    puck.body.setBounce(1,1);
    puck.body.setFriction(10);
 ...
 //   console.log("DATI DEL PUCK= ",puck.x,"-",puck.y,"-",puck.force.x,"-",puck.force.y);
    console.log("PUCK___=",puck.body);
...
function update(){
 //   speed.x-=0.1;
   ...
   // speedPoint.x+=(-1)*(0.1*Math.sign(speedPoint.x));
   // speedPoint.y+=(-1)*(0.1*Math.sign(speedPoint.y));
...
/* 
 ...
 /*    if(speed.x > 0){
        var angle = Math.atan2(app.posY - puck.body.y, app.posX - puck.body.x);  
        var tmp1 = 0- (Math.cos(angle) * speed.x);
        var tmp2 = 0- (Math.sin(angle) * speed.x);
        puck.setVelocityX(tmp1);
        puck.setVelocityY(tmp2);
    } */
   // var angle = Math.atan2(app.posY - puck.body.y, app.posX - puck.body.x);  
    //var tmp1 = Math.sign(Math.cos(angle)) * (Math.cos(angle) * speed);
    //var tmp2 = Math.sign(Math.sin(angle)) *  (Math.sin(angle) * speed);
  //speedPoint.x = speedPoint.x < 0 ? 0 : speedPoint.x;
   // speedPoint.y = speedPoint.y < 0 ? 0 : speedPoint.y;
    
   
  //  puck.setVelocityX(speedPoint.x);
  //  puck.setVelocityY(speedPoint.y);
  //  puck.setAcceleration(speedPoint.x,speedPoint.y);


    textDEBUG.setText(puck.body.velocity.x+','+puck.body.velocity.y,100,100);

    striker1.x = app.rival.posX;
    striker1.y = app.rival.posY;

//    console.log("x---",puck.body.x);
//    console.log("y---",puck.body.y);
//    console.log("Val---",striker2.body);
/*
    puck.x = app.posBallX;
    puck.y = app.posBallY;
*/
...
*/
    ...
    //    puck.setAngularDrag(90);
     ...
     var vect = puck.body.position.subtract(striker2.body.position);
     var direction = vect.normalize();
     direction.x = direction.x * speedMAX;
     direction.y = direction.y * speedMAX;
     puck.body.velocity = direction;


        var angle = Math.atan2(app.posY - puck.body.y, app.posX - puck.body.x);  
        var tmp1 =  (Math.cos(angle) * speedMAX);
        var tmp2 = (-1)*(Math.sin(angle) * speedMAX);
      //  puck.setVelocityX(tmp1);
        //puck.setVelocityY(tmp2);
        speedPoint.x = tmp1;
        speedPoint.y = tmp2;
        console.log("COS+SIN=",tmp1,"-",tmp2);
        console.log(puck);
        //this.physics.accelerateTo(puck, Math.cos(angle), Math.sin(angle) , speedMAX);
        //puck.accelerationFromRotation( angle,speedMAX, puck.acceleration);
        //this.physics.velocityFromAngle(angle, 200, puck.body.velocity) 
        //puck.body.applyForce(tmp1,tmp2); 
    //    puck.body.force.y = Math.sin(angle) * speed;
    })
